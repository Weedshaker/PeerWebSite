...
(function(process) {
  ...
  assert.throws(() => b.write('test', 'utf8', 0), /is no longer supported/);
  ...
  {
    const quote = 'Man is distinguished, not only by his reason, but by this ' + 'singular passion from other animals, which is a lust ' + 'of the mind, that by a perseverance of delight in the ' + 'continued and indefatigable generation of knowledge, ' + 'exceeds the short vehemence of any carnal pleasure.';
    ...
    assert.strictEqual(quote, b.toString('ascii', 0, quote.length));
    const expectedIllegal = expected.slice(0, 60) + ' \x80' + expected.slice(60, 120) + ' \xff' + expected.slice(120, 180) + ' \x00' + expected.slice(180, 240) + ' \x98' + expected.slice(240, 300) + '\x03' + expected.slice(300, 360);
  ...
  assert.strictEqual(Buffer.from('KioqKioqKg==', 'base64').toString(), '*'.repeat(7));
  assert.strictEqual(Buffer.from('KioqKioqKio=', 'base64').toString(), '*'.repeat(8));
  assert.strictEqual(Buffer.from('KioqKioqKioq', 'base64').toString(), '*'.repeat(9));
  assert.strictEqual(Buffer.from('KioqKioqKioqKg==', 'base64').toString(), '*'.repeat(10));
  assert.strictEqual(Buffer.from('KioqKioqKioqKio=', 'base64').toString(), '*'.repeat(11));
  assert.strictEqual(Buffer.from('KioqKioqKioqKioq', 'base64').toString(), '*'.repeat(12));
  assert.strictEqual(Buffer.from('KioqKioqKioqKioqKg==', 'base64').toString(), '*'.repeat(13));
  assert.strictEqual(Buffer.from('KioqKioqKioqKioqKio=', 'base64').toString(), '*'.repeat(14));
  assert.strictEqual(Buffer.from('KioqKioqKioqKioqKioq', 'base64').toString(), '*'.repeat(15));
  assert.strictEqual(Buffer.from('KioqKioqKioqKioqKioqKg==', 'base64').toString(), '*'.repeat(16));
  assert.strictEqual(Buffer.from('KioqKioqKioqKioqKioqKio=', 'base64').toString(), '*'.repeat(17));
  assert.strictEqual(Buffer.from('KioqKioqKioqKioqKioqKioq', 'base64').toString(), '*'.repeat(18));
  assert.strictEqual(Buffer.from('KioqKioqKioqKioqKioqKioqKg==', 'base64').toString(), '*'.repeat(19));
  assert.strictEqual(Buffer.from('KioqKioqKioqKioqKioqKioqKio=', 'base64').toString(), '*'.repeat(20));
  assert.strictEqual(Buffer.from('Kg', 'base64').toString(), '*');
  assert.strictEqual(Buffer.from('Kio', 'base64').toString(), '*'.repeat(2));
  assert.strictEqual(Buffer.from('KioqKg', 'base64').toString(), '*'.repeat(4));
  assert.strictEqual(Buffer.from('KioqKio', 'base64').toString(), '*'.repeat(5));
  ...
  }
  {
    ...
    assert.strictEqual(buf.write(''), 0);
    assert.strictEqual(buf.write('\0'), 1);
    assert.strictEqual(buf.write('a\0'), 2);
    assert.strictEqual(buf.write('あ'), 0);
    assert.strictEqual(buf.write('\0あ'), 1);
    assert.strictEqual(buf.write('\0\0あ'), 2);
  }
  {
    const buf = Buffer.allocUnsafe(10);
    assert.strictEqual(buf.write('あいう'), 9);
    assert.strictEqual(buf.write('あいう\0'), 10);
  }
  {
  ...
  assert.throws(() => Buffer.allocUnsafe(8).writeFloatLE(0.0, 0xffffffff), RangeError);
  assert.throws(() => Buffer.allocUnsafe(8).writeFloatLE(0.0, 0xffffffff), RangeError);
  ...
  common.expectsError(() => Buffer.from('', 'buffer'), {
    code: 'ERR_UNKNOWN_ENCODING',
    type: TypeError,
    message: 'Unknown encoding: buffer'
  });
  {
    const a = [...Array(128).keys()];
  ...
  if (common.hasCrypto) {
    ...
    assert.strictEqual(crypto.createHash('sha1').update(b1).digest('hex'), crypto.createHash('sha1').update(b2).digest('hex'));
  } else {
  ...
  Buffer.poolSize = ps;
  assert.throws(() => Buffer.allocUnsafe(10).copy(), /TypeError: argument should be a Buffer/);
  const regErrorMsg = new RegExp('The first argument must be one of type string, Buffer, ' + 'ArrayBuffer, Array, or Array-like Object\\.');
